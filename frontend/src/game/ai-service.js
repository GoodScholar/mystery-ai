...
如果没有触发：{"triggered":false}`
  ...
  /**
   * 语义评分
   * @param {Object} scenario 剧本信息
   * @param {Object} playerDeduction { suspect, motive, method }
   * @returns {Promise<{motiveScore: number, methodScore: number, comments: string}>}
   */
  async scoreDeduction(scenario, playerDeduction) {
    if (!this.isConfigured) return null

    const prompt = `你是一个公平严格的剧本杀裁判。请根据玩家的推理进行评分（每项满分30分）并给出1句简短评价。
...
`
    ...
    try {
      const response = await this.chat(
        '你是一个精确的JSON分析器。只返回合法的JSON对象，不要包含任何其他文字或标记。',
        [],
        prompt
      )
      const cleaned = response.replace(/```json?\n?/g, '').replace(/```/g, '').trim()
